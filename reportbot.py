...
def start(message: Message):
    ...
    bot.send_message(message.chat.id, f"Welcome to the report bot.", reply_markup=general_kb)
...
def all_messages(message: Message):
    if message.text == "🛍️Order Reports":
        ...
        bot.send_message(message.chat.id, "What Social media do you want to order reports?", reply_markup=kb)
    if message.text == "👤Account":
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("➕Add balance", callback_data="self_add_balance"))
        user = get_user(message.chat.id)
        bot.send_message(message.chat.id,
                        f"<b>Welcome</b>.\n\n🪪Your ID: <code>{message.chat.id}</code>\n💲Balance: ${user.balance}",
                        reply_markup=kb)
    if message.text == "📞Support":
        bot.send_message(message.chat.id, f"Contact support: {support}", reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("📞Support", url=support)))
...
def callback_query_handler(callback: CallbackQuery):
    ...
    if data == "self_add_balance":
        ...
        bot.send_message(message.chat.id, f"What type of report do you want to generate for {social}?", reply_markup=kb)

    elif data.startswith("order:") or data.startswith("plus:") or data.startswith("minus:"):
        report_type, value, social = data.split(":")[1:]
        value = int(value)
        if value < 100:
            bot.answer_callback_query(message.id, "Minimum amount is 100", show_alert=True)
            return
        kb = InlineKeyboardMarkup()
        plus = InlineKeyboardButton("+", callback_data=f"plus:{report_type}:{value+10}:{social}")
        minus = InlineKeyboardButton("-", callback_data=f"minus:{report_type}:{value-10}:{social}")
        num = InlineKeyboardButton(str(value), callback_data="String")
        kb.add(plus, num, minus)
        kb.add(InlineKeyboardButton("Proceed", callback_data=f"proceed:{report_type}:{value}:{social}"))
        bot.edit_message_text("How many reports do you want to generate", message.chat.id, message.id, reply_markup=kb)


    elif data.startswith("proceed:"):
        report_type, value, social = data.split(":")[1:]
        bot.send_message(message.chat.id, f"Send the link to the {social} account you want to generate reports for", reply_markup=cancel_kb)
        bot.register_next_step_handler(message, proceed_to_get_details, *data.split(":")[1:])
        ...
        user.balance -= price
        session.commit()
        bot.edit_message_text(f"Generating {value} reports for {report_type}.\n\n<i>This may take up to 48 hours</i>", message.chat.id, message.id)
        for i in owners:
            bot.send_message(i, f"New order\n\nUser: {message.chat.id}\nSocial: {social}\nReport Type: {report_type}\nReports: {value}\nLink: {link}")
    ...
    if data.startswith("admin_") and message.chat.id in owners:
        ...
        if data == "home":
            ...
            pending_orders = session.query(Order).filter_by(shipped=data != "pending_orders").all()
            kb = InlineKeyboardMarkup()
            kb.add(*[InlineKeyboardButton(order.user, callback_data=f"admin_order_details:{order.id}") for order in pending_orders])
            kb.add(Admin.back_btn("view_orders"))
            if data == "pending_orders":
                bot.edit_message_text("Pending orders", message.chat.id, message.id, reply_markup=kb)
            else:
                bot.edit_message_text("Shipped Orders", message.chat.id, message.id, reply_markup=kb)
            ...
            if not order_details.shipped:
                ...
                kb.add(InlineKeyboardButton("🗑Delete Order", callback_data="admin_del_order:" + str(orderid)))
            kb.add(Admin.back_btn("pending_orders") if not order_details.shipped else Admin.back_btn("shipped_orders"))
...
def self_add_balance(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Send the amount as a number (e.g 100, 200)", reply_markup=cancel_kb)
        bot.register_next_step_handler(message, self_add_balance)
        return
    if amount < 4:
        bot.send_message(message.chat.id, "Minimum amount is $4. Try again!", reply_markup=cancel_kb)
        bot.register_next_step_handler(message, self_add_balance)
        return
...
def proceed_to_get_details(message, report_type, value, social):
    ...
    link = message.text
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("Confirm", callback_data=f"pay:{report_type}:{value}:{social}:{link}"))
